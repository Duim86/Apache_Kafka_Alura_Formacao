...
public class KafkaDispatcher<T> implements Closeable {
    ...
    public void send(String topic, String key, CorrelationId id, T payload) throws ExecutionException, InterruptedException {
        var future = sendAsyc(topic, key, id, payload);
        future.get();
    }

    public Future<RecordMetadata> sendAsyc(String topic, String key, CorrelationId id, T payload) {
        var value = new Message<>(id.continueWith("_" + topic), payload);
        var recorde = new ProducerRecord<>(topic, key, value);
        ...
        return producer.send(recorde, callback);
    }
